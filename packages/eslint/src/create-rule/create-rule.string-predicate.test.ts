...
const lint = (rule: unknown, code: string): string[] =>
  new Linter()
    .verify(
      code,
      {
...
describe('createRule with string predicates', () => {
  it(
    'applies a .when over a U.or capture once, for either spelling of the ' +
      'key',
    () => {
      expect(lint(renamedKey, 'const { a: b } = o;')).toEqual([
        'a renamed to b',
      ])
      expect(lint(renamedKey, "const { 'a': b } = o;")).toEqual([
        'a renamed to b',
  ...
  it('tests a slot with a predicate the way it does with a RegExp', () => {
    ...
    expect(
      lint(U.Identifier({ name: /^ki/ }).message('re'), 'o.kind;'),
    ).toEqual(['re'])
  })
})

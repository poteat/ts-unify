 ...
 */
const DIRECTIVE =
  /^\s*(oxlint-|eslint-|eslint\s|eslint$|@ts-(expect-error|ignore|nocheck|check)\b|prettier-ignore|biome-ignore|c8 ignore|v8 ignore|istanbul ignore)/
const LICENSE = /\b(copyright|licen[cs]e|spdx)\b/i

/**
 * Every comment, except directives, a license header and a JSDoc on a
 * declaration.
 */
const noInlineComments = U.or(
  U.Comment({
    ...
    text: $('text'),
    header: $('header'),
  }).when(it => !DIRECTIVE.test(it.text)),
  U.Comment({
    kind: 'jsdoc',
    attachedTo: null,
    text: $('text'),
  ...
  .message(
    'a comment here means this code wants a name: extract it to a binding ' +
      'and put the explanation in its JSDoc',
  )

/**
 * Summary max 2 lines, body max 3 with one blank before it, each tag max 2
 * lines, nothing after the tags.
 */
const jsdocShape = U.or(
  ...
  'JSDoc overflows: summary max 2, body max 3, each tag max 2, nothing after ' +
    'the tags',
)

/**
 * Every source line of a JSDoc block fits the file's 80 columns; the first line
...
const jsdocLineWidth = U.Comment({
  ...
  .when(({ lines, loc }) =>
    lines.some((l, i) => (i === 0 ? loc.start.column : 0) + l.length > 80),
  )
 ...
 */
const identifierTokens = (name: string): Set<string> =>
  new Set(
    name
...
function restates(name: unknown, summary: string) {
  ...
  return (
    ...
    words.every(
      w =>
        tokens.has(w) || tokens.has(w.replace(/s$/, '')) || tokens.has(w + 's'),
    )
  )
}
    ...
    .verify(
      code,
      {
        ...
        plugins: {
          t: {
            rules: {
              r: createRule(rule as never) as never,
            },
          },
        },
...
function valid(rule: unknown, codes: string[]) {
  for (const code of codes)
    expect({ code, reports: lint(rule, code) }).toEqual({ code, reports: [] })
}

function invalid(rule: unknown, codes: string[]) {
  for (const code of codes)

...
describe('createRule', () => {
  ...
  it('creates a visitor for the correct AST node type', () => {
    const visitors = createRule(ifStmt).create({
      report() {},
    } as any)
  ...
  it(
    'runs a Comment entry from the Program visitor and reports at the ' +
      "comment's loc",
    () => {
      const rule = createRule(U.Comment({ kind: 'line' }))
      const reported: any[] = []
      const visitors = rule.create({
        report: (d: any) => reported.push(d),
      } as any)
  ...
  it('visitor does not report when a node does not match', () => {
    ...
    const rule = createRule(U.Identifier({ name: 'specific' }) as any)
  ...
  it('resolves config slots in imports from the chain config defaults', () => {
    ...
    rule
      ...
      .ArrayExpression(fakeNode as any)
  ...
  it('honours a .when() on a root or after the branch matched', () => {
    const rule = createRule(
      (U as any)
        .or(
          (U as any).Identifier({ name: $('n') }),
          (U as any).Literal({ value: $('n') }),
        )
        .when((it: { n: unknown }) => it.n !== 'skip'),
    )
    const reported: any[] = []
    const visitors = rule.create({
      report: (d: any) => reported.push(d),

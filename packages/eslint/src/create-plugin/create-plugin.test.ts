...
describe('createPlugin', () => {
  ...
  it('uses qualified kebab names in the recommended config', () => {
    ...
    ).not.toHaveProperty('ts-unify/my-rule')
  })
  ...
  it('uses default message when rule has no .message()', () => {
    expect(
      createPlugin({
        myRule: idPattern,
      }).rules['my-rule'].meta.messages.match,
    ).toBe('Matches a ts-unify pattern')
  })

  it('handles an empty rules map', () => {
    expect(Object.keys(createPlugin({}).rules)).toHaveLength(0)
  })
})

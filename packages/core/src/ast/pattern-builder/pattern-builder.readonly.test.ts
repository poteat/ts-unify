...
describe("PatternBuilder readonly shape", () => {
  ...
  it("keeps the capture bag writable", () => {
    function check(u: BuilderMap) {
      const p = u.Identifier({ name: $("n") });
      assertType<ExtractCaptures<typeof p>, { n: string }>(0);
    }
    void check;
  });

  it("accepts Object.freeze around a builder call", () => {
    const frozen = Object.freeze(U.Identifier({ name: $("n") }));
    assertType<ExtractCaptures<typeof frozen>, { n: string }>(0);
    const narrowed = frozen.when((n): n is "x" => n === "x");
    assertType<ExtractCaptures<typeof narrowed>, { n: "x" }>(0);
    const built = Object.freeze(U.Identifier({ name: "x" }));
    const any = Object.freeze(U.Identifier());

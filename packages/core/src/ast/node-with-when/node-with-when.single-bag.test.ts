...
describe("NodeWithWhen bag form on a single-capture node (type-level)", () => {
  it("accepts an annotated bag guard over a U.or and narrows the capture where it is embedded", () => {
    ...
    assertType<Bag["key"], string>(0);
    assertType<Bag["name"], string>(0);

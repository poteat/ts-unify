...
const between = (first: string, last: string): string[] => {
  const kinds = ts.SyntaxKind as unknown as Record<string, number>;
  return Object.entries((ts as unknown as { textToKeywordObj: Record<string, number> }).textToKeywordObj)
...
describe("reserved", () => {
  it("matches the installed TypeScript's keyword table", () => {
    expect(RESERVED_WORDS).toEqual(between("FirstReservedWord", "LastReservedWord"));
    expect(STRICT_MODE_RESERVED_WORDS).toEqual(between("FirstFutureReservedWord", "LastFutureReservedWord"));
    expect(CONTEXTUAL_KEYWORDS).toEqual(between("FirstContextualKeyword", "LastContextualKeyword"));
  });

  it("always counts the ECMAScript reserved words", () => {
    for (const word of ["break", "class", "this", "null", "true", "enum", "in"]) {
  ...
  it("counts the strict-mode words and await unless strict is false", () => {
    for (const word of ["let", "yield", "static", "implements", "interface", "await"]) {
      expect(reserved()(word)).toBe(true);

...
describe("match - string predicates", () => {
  it("tests a string position against a RegExp", () => {
    ...
    expect(match(id("bar"), U.Identifier({ name: U.string.regex(/^b/) }))).toEqual({});
  });

  it("never matches a non-string", () => {
    expect(match({ type: "Literal", value: 42 }, U.Literal({ value: /4/ }))).toBeNull();
    expect(match({ type: "Literal", value: 42 }, U.Literal({ value: U.string.identifierName() }))).toBeNull();
  });
  ...
  it("matches a reserved word with U.string.reserved and not a near miss", () => {
    ...
    expect(match(id("let"), U.Identifier({ name: U.string.reserved({ strict: false }) }))).toBeNull();
    expect(match(id("type"), U.Identifier({ name: U.string.reserved({ typescript: true }) }))).toEqual({});
  });

  it("matches an IdentifierName key and negates with U.string.not", () => {
    const quoted = (value: string) => ({ type: "Property", key: { type: "Literal", value }, value: id("v") });
    const needless = U.Property({ key: U.Literal({ value: U.string.identifierName() }) });
  ...
  it("applies a .when over a U.or capture once, whichever branch bound it", () => {
    const key = U.or(U.Identifier({ name: $("key") }), U.Literal({ value: $("key") })).when(
    ...
    expect(match(prop({ type: "Literal", value: "name" }), p)).toEqual({ key: "name" });

...
export function jsdocParts(lines: readonly string[]): {
  ...
  while (Lines.isParagraphLine(lines, i)) summary.push(lines[i++])
  ...
  if (i < lines.length && lines[i] === '') {
    ...
    while (i < lines.length && !Lines.isTagLine(lines[i])) body.push(lines[i++])
  ...
  while (i < lines.length) {
    const line = lines[i++]
    if (!Lines.isTagLine(line)) continue
    const space = line.search(/\s/)
    const name = space === -1 ? line : line.slice(0, space)
    const text = [space === -1 ? '' : line.slice(space + 1).trimStart()]

    while (Lines.isParagraphLine(lines, i)) text.push(lines[i++].trimStart())

import type { CommentNode, CommentKind, JsdocTag } from "@ts-unify/core/internal";
...
const cache = new WeakMap<object, { list: CommentNode[]; byRaw: WeakMap<object, CommentNode> }>();
 ...
 */
export function commentNodes(program: unknown): CommentNode[] {
  return views(program).list;
}

/** The `Comment` node of one raw parser comment of `program`, if any. */
export function commentNodeOf(program: unknown, raw: unknown): CommentNode | undefined {
  if (!raw || typeof raw !== "object") return undefined;
  return views(program).byRaw.get(raw);
}

function views(program: unknown): { list: CommentNode[]; byRaw: WeakMap<object, CommentNode> } {
  ...
  const list = raws.map((raw, index) => {
    const header = index === 0 && raw.range[1] <= firstToken;
    const node = toCommentNode(raw, header, p, starts, tokenStarts);
    byRaw.set(raw, node);
    return node;
  });
...
function toCommentNode(
  ...
  const kind: CommentKind = raw.type === "Line" ? "line" : raw.value.startsWith("*") ? "jsdoc" : "block";
...
/** Lines of a JSDoc value with the leading `*` per line stripped and blank edges dropped. */
...
const isTagLine = (l: string): boolean => /^@\w/.test(l);

function jsdocParts(lines: string[]): { summary: string[]; body: string[]; tags: JsdocTag[] } {
  let i = 0;
  const summary: string[] = [];
  while (i < lines.length && lines[i] !== "" && !isTagLine(lines[i])) summary.push(lines[i++]);
  const body: string[] = [];
  ...
  while (i < lines.length) {
    ...
    while (i < lines.length && lines[i] !== "" && !isTagLine(lines[i])) text.push(lines[i++].trimStart());

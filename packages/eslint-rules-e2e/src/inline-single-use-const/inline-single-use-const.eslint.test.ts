...
tester.run(
  ...
  }),
  {
    valid: [
      ...
      'function f() { const runs: number[] = []; return { keep: (n: number) ' +
        '=> runs.push(n), count: () => runs.length }; }',
      // A read in a loop's test
      'function f(ms: number) { const deadline = Date.now() + ms; while ' +
        '(Date.now() < deadline) {} }',
      // A read in a loop's body
      'function f(xs: number[], abort: (e: Error) => void) { const reason = ' +
        "new Error('x'); for (const x of xs) abort(reason); }",
      // A read inside a callback
      'function f(make: () => Promise<number>, later: (cb: () => ' +
        'Promise<number>) => void) { const held = make(); later(async () => ' +
        'await held); }',
      // A typeof of the name
      'function f(make: () => { a: number }) { const first = make(); const ' +
        'second: typeof first = { a: first.a + 1 }; return [second]; }',
      // A same-named parameter after the declaration
      'function f(seed: () => number, xs: number[]) { const x = seed(); ' +
        'return xs.map(x => x + 1); }',
      ...
      'function f(name: string) { const head = `export function ${name}`; ' +
        'return `${head}(): void`; }',
      // A shorthand property
      'function f(make: () => number) { const n = make(); return { n }; }',
      // An effectful initializer after another effect
      'function f(a: () => number, b: () => number, use: (x: number, y: ' +
        'number) => void) { const x = a(); use(b(), x); }',
    ],
    invalid: [
      {
        code:

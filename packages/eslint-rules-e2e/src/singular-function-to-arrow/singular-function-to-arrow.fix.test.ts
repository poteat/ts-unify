...
/** Run a rule over parsed source and return each report's fix text, whitespace collapsed. */
function fixes(src: string): string[] {
  ...
  const sourceCode = {
    ast,
    text: src,
    getText: (n?: { range: [number, number] }) => (n ? src.slice(n.range[0], n.range[1]) : src),
  };
...
describe("createRule fixes on TypeScript nodes", () => {
  it("reprints a function type in the return annotation", () => {
    expect(fixes("function f(): { a?: (x: string) => string } { return {} }")).toEqual([
      "const f = (): { a?: (x: string) => string; } => ({});",
    ]);
  });

  it("keeps type arguments on a type reference", () => {
    expect(fixes("function f() { return globalThis as unknown as ReturnType<typeof f> }")).toEqual([

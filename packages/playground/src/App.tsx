              ...
              <Panel defaultSize={60} minSize={20}>
                <section className="panel">
                  <div className="panel-header">
                    <div className="tabs">
                      <button
                        className={`tab ${leftTab === 'rules' ? 'active' : ''}`}
                      ...
                      <>
                        <div className="rule-list">
                          {ALL_RULES.map(r => (
                            <div
                              key={r.kebab}
                              className={`rule-item ${enabledRules.has(r.kebab) ? 'enabled' : 'disabled'}`}
                ...
                <section className="panel">
                  <div className="panel-header">
                    <div className="tabs">
                      ...
                      <button
                        className={`tab ${sourceTab === 'ast' ? 'active' : ''}`}
                        onClick={() => setSourceTab('ast')}
                      >
                        AST
                      </button>
                    </div>
                  ...
                  <div
                    className={`panel-body ${sourceTab === 'source' ? 'no-scroll' : ''}`}
                  >
                    {sourceTab === 'source' ? (
                      ...
                      <HighlightedCode
                        code={JSON.stringify(ast, astReplacer, 2)}
                        monaco={monacoApi}
                        language="json"
                      />
                    )}
                ...
                <section className="panel">
                  <div className="panel-header">
                    <div className="tabs">
                      ...
                      <button
                        className={`tab ${outputTab === 'ast' ? 'active' : ''}`}
                        onClick={() => setOutputTab('ast')}
                      >
                        AST
                      </button>
                      <button
                        className={`tab ${outputTab === 'diff' ? 'active' : ''}`}
...
function HighlightedCode({
  ...
  const lineSpans = useMemo(
    () => lines.map(l => Theme.tokenizeLine(monaco, l, language)),
    [lines, monaco, language],
  )
...
function DiffView({
  ...
  const rowSpans = useMemo(
    () => rows.map(r => Theme.tokenizeLine(monaco, r.line, 'typescript')),
    [rows, monaco],
  )
  ...
  return (
    <div className="diff-view">
      {rows.map((row, i) => (
        <div
          key={i}
          className={`diff-line ${row.kind === 'add' ? 'add' : row.kind === 'del' ? 'del' : ''}`}
        >

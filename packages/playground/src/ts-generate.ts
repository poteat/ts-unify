...
type S = { readonly write: (s: string, node?: any) => void }
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type N = any
...
const tsGenerator: Record<string, any> = {
  ...
  TSTypeAnnotation(node: N, state: S) {
    state.write(': ')
    this[node.typeAnnotation.type](node.typeAnnotation, state)
  },
  TSStringKeyword(_n: N, state: S) {
    state.write('string')
  },
  TSNumberKeyword(_n: N, state: S) {
    state.write('number')
  },
  TSBooleanKeyword(_n: N, state: S) {
    state.write('boolean')
  },
  TSVoidKeyword(_n: N, state: S) {
    state.write('void')
  },
  TSAnyKeyword(_n: N, state: S) {
    state.write('any')
  },
  TSUnknownKeyword(_n: N, state: S) {
    state.write('unknown')
  },
  TSNeverKeyword(_n: N, state: S) {
    state.write('never')
  },
  TSNullKeyword(_n: N, state: S) {
    state.write('null')
  },
  TSUndefinedKeyword(_n: N, state: S) {
    state.write('undefined')
  },
  TSBigIntKeyword(_n: N, state: S) {
    state.write('bigint')
  },
  TSSymbolKeyword(_n: N, state: S) {
    state.write('symbol')
  },
  TSObjectKeyword(_n: N, state: S) {
    state.write('object')
  },
  ...
  TSQualifiedName(node: N, state: S) {
    this[node.left.type](node.left, state)
    state.write('.')
    state.write(node.right.name)
  },
  TSUnionType(node: N, state: S) {
    for (let i = 0; i < node.types.length; i++) {
      if (i > 0) state.write(' | ')
      this[node.types[i].type](node.types[i], state)
    }
  },
  TSIntersectionType(node: N, state: S) {
    for (let i = 0; i < node.types.length; i++) {
      if (i > 0) state.write(' & ')
      this[node.types[i].type](node.types[i], state)
    }
  },
  TSArrayType(node: N, state: S) {
    this[node.elementType.type](node.elementType, state)
    state.write('[]')
  },
  TSTupleType(node: N, state: S) {
    state.write('[')

    for (let i = 0; i < node.elementTypes.length; i++) {
      if (i > 0) state.write(', ')
      this[node.elementTypes[i].type](node.elementTypes[i], state)
    }

    state.write(']')
  },
  TSLiteralType(node: N, state: S) {
    node.literal.type === 'Literal'
      ? this.Literal(node.literal, state)
      : this[node.literal.type](node.literal, state)
  },
  TSTypeLiteral(node: N, state: S) {
    state.write('{ ')

    for (let i = 0; i < node.members.length; i++) {
      if (i > 0) state.write('; ')
      this[node.members[i].type](node.members[i], state)
    }

    state.write(' }')
  },
  TSPropertySignature(node: N, state: S) {
    if (node.readonly) state.write('readonly ')
    this[node.key.type](node.key, state)
    if (node.optional) state.write('?')
    if (node.typeAnnotation) this.TSTypeAnnotation(node.typeAnnotation, state)
  },
  TSFunctionType(node: N, state: S) {
    state.write('(')

    if (node.params) {
      for (let i = 0; i < node.params.length; i++) {
        if (i > 0) state.write(', ')
        this[node.params[i].type](node.params[i], state)
      }
    }
  ...
  TSParenthesizedType(node: N, state: S) {
    state.write('(')
    this[node.typeAnnotation.type](node.typeAnnotation, state)
    state.write(')')
  },
  TSTypeQuery(node: N, state: S) {
    state.write('typeof ')
    this[node.exprName.type](node.exprName, state)
  },
  TSAsExpression(node: N, state: S) {

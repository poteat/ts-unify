 ...
 */
function notARead(id: Node, parent: Node | null) {
  ...
  return (
    ((p.type === 'Property' ||
      ...
      p.key === id &&
      !p.computed) ||
    (p.type === 'MemberExpression' && p.property === id && !p.computed) ||
    (p.type === 'VariableDeclarator' && p.id === id) ||
    (FUNCTIONS.has(p.type) && Array.isArray(p.params) && p.params.includes(id)
      ? true
      : p.type === 'ArrayPattern' ||
        p.type === 'ObjectPattern' ||
        p.type === 'RestElement' ||
        (p.type === 'AssignmentPattern' && p.left === id) ||
        (p.type === 'CatchClause' && p.param === id) ||
        ...
        (p.type.startsWith('TS') ? true : false))
  )
}
 ...
 */
function rebinds(tree: unknown, name: string) {
 ...
 */
const namedInType = (tree: unknown, name: string) =>
  [...walk(tree)].some(
    ([n, parent]) =>
      n.type === 'Identifier' &&
      n.name === name &&
...
function readsOf(tree: unknown, name: string): { node: Node; above: Node[] }[] {
  ...
  function visit(t: unknown, above: Node[]): void {
    if (Array.isArray(t)) {
    ...
    if (t.type === 'Identifier' && t.name === name) {
      if (!notARead(t, above[above.length - 1] ?? null))
        found.push({
          node: t,
          above,
        })
 ...
 */
function moves(read: { node: Node; above: Node[] }, stmt: Node, init: Node) {
  const { node, above } = read
  const parent = above[above.length - 1]
  if (parent !== undefined && parent.type === 'Property' && parent.shorthand)
    return true
...
export function inlinableConst(
  body: unknown,
): { index: number; name: string; init: Node; read: Node } | null {
  if (!Array.isArray(body)) return null

  for (let i = 0; i + 1 < body.length; i++) {
    const s = body[i]
    if (!isNode(s) || s.type !== 'VariableDeclaration' || s.kind !== 'const')
      continue
    const decls = s.declarations
    if (!Array.isArray(decls) || decls.length !== 1 || !isNode(decls[0]))
      continue
    const d = decls[0]
    const id = d.id
    const init = d.init
 ...
 */
function substituted<T>(tree: T, target: Node, replacement: Node): T {
...
/**
 * Inline a `const` binding whose one read is in the very next statement: the
 * declaration goes, and that read becomes the initializer.
 *
 * Every single-declarator `const` of the block is examined, first to last. A
 * read is counted by its role, not its name: a key, a member name, a binding, a
 * label or a type position is none; a same-named binding, or a `typeof` of the
 * name, anywhere after the declaration stops the inlining. The one read may not
 * sit inside a nested function or a loop (the initializer would run later, or
 * again), nor be a shorthand property, nor a template literal's hole (a
 * template is built from named fragments); and an initializer with effects may
 * not be moved past another effect or under a branch. An annotated const keeps
 * its check.
 *
 * @example
 * ```ts

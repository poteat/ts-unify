...
import { booleanShaped } from './boolean-shaped'
import { isLiteral } from './is-literal'
import { literalBoolean } from './literal-boolean'
...
export function form(bag: {
  ...
  const consequentLiteral = literalBoolean(consequent)
  const alternateLiteral = literalBoolean(alternate)
    ...
    : consequentLiteral === true && alternateLiteral === false
      ...
      : consequentLiteral === false && alternateLiteral === true
        ...
        : consequentLiteral === true
          ...
          : alternateLiteral === false && !isLiteral(consequent)
            ? U.LogicalExpression({
                operator: '&&',
                left: test as never,
                right: consequent as never,
              })
            : consequentLiteral === false && !isLiteral(alternate)
              ? U.LogicalExpression({
                  operator: '&&',
